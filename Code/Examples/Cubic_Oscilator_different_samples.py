...
class ParametersSetting:
    ...
    add_noise: bool = True
    noise_level = list(np.arange(0.00, 0.07, 0.02))
    useRK = [False, True]
    useNN = [False, True]
    ...
    sys_model: str = "Cubic Damped Oscillator"
...
with open(
    f"{param_set.save_model_path}" + "er_curves_diff_samples_NNRK_list", "wb"
) as fp:
    pickle.dump(er_curves_samples_NNRK_list, fp)


with open(
    f"{param_set.save_model_path}" + "er_curves_diff_samples_NN_list", "wb"
) as fp:
...
with open(
    f"{param_set.save_model_path}" + "er_curves_diff_samples_NNRK_list", "rb"
) as fp:
    er_curves_samples_NNRK_list = pickle.load(fp)


with open(
    f"{param_set.save_model_path}" + "er_curves_diff_samples_NN_list", "rb"
) as fp:
...
param_heatmap = HeatmapSetting(v_max = 4, fontsize = 12)
