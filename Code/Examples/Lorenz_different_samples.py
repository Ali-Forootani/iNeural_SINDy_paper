...
# network = Siren(int(param_set.num_indp_var+1), [param_set.num_hidden_neur, param_set.num_hidden_neur, param_set.num_hidden_neur], int(param_set.num_indp_var)).to(device)
...
#                         time_deriv_coef=2/(param_set.t_max - param_set.t_min)).to(device)
...
for l in range(len(param_set.noise_level)):
    for i in range(len(param_set.num_samples)):
        ...
        x_3_er_sens_array_NNRK[l, i] = er_curves_samples_NNRK_list[i][l][0][3]

        x_1_er_sens_array_NN[l, i] = er_curves_samples_NN_list[i][l][0][1]
        x_2_er_sens_array_NN[l, i] = er_curves_samples_NN_list[i][l][0][2]
        x_3_er_sens_array_NN[l, i] = er_curves_samples_NN_list[i][l][0][3]
...
"""
param_heatmap = HeatmapSetting(v_max = 22, fontsize = 15)
...
with open(
    f"{param_set.save_model_path}"
    + "er_curves_samples_NNRK_list_diff_samples"
    + "_scaling_factor_"
    + str(param_set.fun_scaling_factor),
    "wb",
) as fp:
...
with open(
    f"{param_set.save_model_path}"
    + "er_curves_samples_NN_list_diff_samples"
    + "_scaling_factor_"
    + str(param_set.fun_scaling_factor),
    "wb",
) as fp:
