...
list_initial_conditions = [[2, 2], [-2, -2], [2, -2]]
...
coeff_noise_list = [] # it stores the estimated coeffs
dic_coef_track_list = []
        ...
        for j in range(len(param_set.useNN)):
            if param_set.useRK[k] == False and param_set.useNN[j] == False:
                ...
                estimated_coeffs_k = CoeffsNetwork(
                    coef_dim_row, param_set.num_indp_var
                ).to(device)
...
plt.savefig(
    ...
    + "2_D_Oscillatory_training_data_noise_"
...
with open(
    ...
    pickle.dump(dic_coef_track_list, fp)
...
if param_set.tikz_save:
    tikzplotlib.save(param_set.save_model_path + "2_D_Oscillatory_loss.tex")
plt.savefig(
    param_set.save_model_path
    + "2_D_Oscillatory_loss_noise_"
    + str(param_set.noise_level[0])
    + ".png"
)
...
"""plotting the curves corresponding to different algorithms """
...
axs[0].set_xlim((-2.6, 2.9))
axs[0].set_ylim((-2.75, 2.5))

axs[1].set_xlim((-2.6, 2.9))
axs[1].set_ylim((-2.75, 2.5))

axs[2].set_xlim((-2.6, 2.9))
axs[2].set_ylim((-2.75, 2.5))
...
""" 
Ploting the coefficients through the iterations
"""
...
# fig.suptitle("Linear Oscillator", fontsize=30, y=1)

axs[0].plot(
    track_coef_matrix_1_NNRK[:7500, 1],
    color=colors[0],
    marker="^",
    linestyle="-",
    markersize=10,
...
axs[1].plot(
    track_coef_matrix_1_NNRK[:7500, 2],
    color=colors[0],
    marker="^",
    linestyle="-",
    markersize=10,
...
axs[2].plot(
    track_coef_matrix_2_NNRK[:7500, 1],
    color=colors[0],
    marker="^",
    linestyle="-",
    markersize=10,
...
# axs[2].set(ylabel='Coefficients')
axs[2].set(xlabel="Iteration")

axs[3].plot(
    track_coef_matrix_2_NNRK[:7500, 2],
    color=colors[0],
    marker="^",
    linestyle="-",
    markersize=10,
