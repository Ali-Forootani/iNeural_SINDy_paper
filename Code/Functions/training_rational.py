...
from Functions.linear_coeffs_module import (rk4th_onestep_SparseId,
...
def train(
    ...
    for key in func_arguments:
        ...
        if key == "useRK":
            useRK = func_arguments[key]
        if key == "useNN":
            useNN = func_arguments[key]
    ...
    for iteration in range(max_iterations):
        ...
        loss_prediction_target = torch.autograd.Variable(torch.tensor([0.],requires_grad=True)).to(device)
        loss_RK4 = torch.autograd.Variable(torch.tensor([0.],requires_grad=True)).to(device)
        loss_time_coeff = torch.autograd.Variable(torch.tensor([0.],requires_grad=True)).to(device)
        ...
        for i in range(len(train_dataloader)):
            ...
            for batch_idx, train_sample in enumerate(train_dataloader[i]):
                ...
                loss_time_coeff += loss_fn(time_derivs.reshape(-1,total_coeffs.shape[-1]), total_coeffs)
                #loss_RK4 += torch.mean((prediction[1:] - RK4_pred) ** 2)
