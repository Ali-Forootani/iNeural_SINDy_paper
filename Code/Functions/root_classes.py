...
class Constraint(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, input):
        ...
        if self.sparsity_masks is None:
            self.sparsity_masks = torch.ones(thetas.shape[1], dtype=torch.bool).to(
                thetas.device
            )
...
class Estimator(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, thetas, time_derivs):
        ...
        with torch.no_grad():
            normed_time_derivs = (
                (time_derivs / torch.norm(time_derivs)).detach().cpu().numpy()
            )
            normed_thetas = (
                (thetas / torch.norm(thetas, dim=0, keepdim=True))
                .detach()
                .cpu()
                .numpy()
            )
        ...
        sparsity_masks = (
            torch.tensor(self.coeff_vectors != 0.0, dtype=torch.bool)
            .squeeze()
            .to(thetas[0].device)
        )
...
class Library(nn.Module):
    ...
    def forward(self, input):
        ...
        self.norms = (
            (torch.norm(time_derivs) / torch.norm(thetas, dim=0, keepdim=True))
            .detach()
            .squeeze()
        )
